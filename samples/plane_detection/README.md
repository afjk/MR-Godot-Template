...
**このAPIが返すのは、多くの端末では「事前にスキャンした部屋のデータ」です。** OpenXRの仕様は取得元を区別しないので、runtimeによってはリアルタイム検出が返ることもありますが、Quest 3ではSpace Setupの結果になります。机を動かしても平面は動きません。

その場で検出したい場合は[リアルタイム平面推定](../realtime_planes/)を見てください。深度マップから毎回作り直すので、スキャン不要で動かした物にも追従します（精度は落ちます）。
