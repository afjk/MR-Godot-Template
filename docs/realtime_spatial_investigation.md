...
### S1: `realtime_mesh`（環境メッシュの表示）※実装済み

- `[shader_globals]`を整備し、格子メッシュ＋頂点シェーダーで**毎フレーム更新される環境メッシュ**を出す
- 表示モードはpinchで切替（格子線／面）
- **これで「メッシュ生成」は目に見える形で達成できます**。CPUを使わないので性能面の心配もありません

> 実装時のメモ: オクルージョン用のモードは**入れませんでした**。深度によるオクルージョンは、メッシュ化を経由せず仮想物体側のフラグメントシェーダーで比較する[`occlusion_depth`](../samples/occlusion_depth/)のやり方のほうが正確で、既に動いています。1サンプル1テーマの規約からも、こちらは「メッシュを作る」ことだけを見せます。
>
> 実装で外せなかったのは4点です。`[shader_globals]`の宣言、頂点シェーダーでの`textureLod`（画面上の微分が無いため`texture()`ではミップを選べない）、`extra_cull_margin`（頂点を動かしてもAABBは元のまま）、そして逆行列が返すのがXRの基準空間だという点（ノードを`XROrigin3D`直下に置いてモデル空間と一致させました）。

### S2: `realtime_mesh_collision`（当たり判定）※実装済み

- CPU経路で深度を落とし、間引いた格子から`ArrayMesh`と`ConcavePolygonShape3D`を作る
- 1 Hz更新。ボールを投げて実際の床や机で跳ねるところまで

> 実装時のメモ: 表示だけなら物の縁で三角形が引き伸ばされても許せますが、**当たり判定では見えない板として効いてしまう**ため、四隅が0.2mより離れた四角は捨てています。S1のGPU版より厳しくしてあるのはそのためです。間引きは40×40（三角形は最大3,042枚）。1秒に1回とはいえ、その1回が長いと引っかかりとして見えるので、増やすなら実測が要ります。

### S3: `realtime_plane_clusters`（複数平面の検出）※実装済み

- 第5節の手順を実装。検出した平面を色分け＋境界表示、フレーム間で追跡
- ここまで来ると、**Quest 3で「動かした机がその場で平面として出る」**が実現します

> 実装時のメモ: 平面フィットにPCAは要りませんでした。**法線は点ごとに既に出してあるので、まとまりの中で平均するだけで足ります。** 共分散行列を回すのは、点しか無い場合の話です。
>
> 境界は平面座標へ射影した軸並行の矩形にしています。凸包のほうが実際の形に近くなりますが、コード量が大きく増えるわりに、サンプルとして見せたいこと（複数の面が別々に取れて、追跡できている）は矩形で十分伝わります。
>
> GDScriptの落とし穴を1つ。**`PackedInt32Array`は値型なので、関数に渡すと複製されます。** 領域拡張の訪問済みフラグをこれで持つと、印が呼び出し元へ戻らず、同じ点が何度も種になります。`Array[bool]`（参照型）にしました。
>
