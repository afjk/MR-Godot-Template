...
> **位置づけの更新**: このリポジトリはサンプル集として育てる方針になりました（[docs/samples_plan.md](samples_plan.md)）。そのため、**toolkitを先に作るのではなく、まず各機能をサンプルとして書き、繰り返し現れたものを共通コードへ抽出する**進め方に変更します。以下のアーキテクチャ案とAPI案は、その抽出先の設計として引き続き有効です。M0-M5のロードマップは、サンプル側の進め方（`samples_plan.md`第8節）に読み替えてください。


> **26サンプル書いた時点での結論（追記）**: このメモが設計したinteractor / interactableの抽象は、**まだ一度も必要になりませんでした**。各サンプルの主題はその当たり判定そのもので、対象は数個。MRTK3の調停が要るのは「多数のinteractableを登録して、複数のinteractorが取り合う」段階からです。実際に`shared/`へ抽出されたのは、MR土台・pinch判定・ポインタの出所・深度の座標変換だけでした。判断の記録は[サンプル集の方針](samples_plan.md)第9.5節にあります。
